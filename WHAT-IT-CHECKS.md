...
**Visual and product-direction changes are different: you settle the look live, before it is even written down.** `npm run preview` boots a real, seeded copy of the app and hands you a localhost link. You keep that open while the build edits the real front end directly against it — you refresh, say "arrows are clutter," refresh again, and repeat until you say approved. Only then does the pipeline freeze what you approved, write it down as the acceptance criteria, and run the normal issue-review / implementation / adversarial-review / merge pipeline on it. Nothing is left open waiting on you to click merge yourself, and nothing you approve gets quietly redecorated later without asking you first — the freeze plus the "two doors" rule (`agents/orchestrator.md` § "Visual-approval loop") see to that.
