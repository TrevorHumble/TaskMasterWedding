...
There is no run-tier classifier and no same-model review carve-out: every reviewer, on every issue, runs on a different, non-weaker model than the implementer.
