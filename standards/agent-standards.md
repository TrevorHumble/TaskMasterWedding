...
**`sonnet-only` tier carve-out (#427).** Same-model review is permitted on **exactly one** tier: an issue eligible under `tools/classify-issue-run.ps1` (routine, off the wedding-critical guest paths, small and reversible) may run its implementer and every reviewer that fires on Sonnet. This is a bounded acceptance of reduced model-independence, not a repeal of the rationale above — same-model review does inherit correlated blind spots, and that cost is accepted here only because the tier is restricted to low-stakes, reversible work and the differently-chartered design-philosophy lens still runs (a different lens, even on the same model, catches what a same-charter panelist misses — see #201 in `standards/adversarial-review-protocol.md`). Every reviewer charter that can run on this tier carries a coverage-first instruction: Sonnet follows "be conservative" phrasing literally and under-reports, so the mitigation is non-suppression plus severity/confidence tagging, not model diversity. Any eligibility gate tripping mid-run escalates the remainder to the standard Opus policy. Every other tier keeps the unmodified rule: reviewers run on a different, non-weaker model than the implementer.
