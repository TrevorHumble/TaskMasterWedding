...
**Amended (issue #1103):** the "host-chosen task worth 1/2/3 (#682)" value in the nine-source list above is historical: worth is now host-chosen 3/4/5, default 3, rescaled by #1103. The full rescale record (migration, remap, and what stayed unchanged) lives in this file's "Point-system rebalance (2026-08-04)" section.
...
## Admin event stats page: two-level day/hour chart, union scope rule, duplicate-weekday label fallback (#1022)

**Date:** 2026-08-02. **Status:** shipped.

**Why the chart is two levels (day, then hour), not one flat hour-of-day bucketing.** The issue's original shape bucketed every guest join and photo by plain hour-of-day (0-23), collapsing Friday 7pm, Saturday 7pm, and Sunday 7pm into the same bar — meaningless for a 3-day event where "when did people show up" is a per-day question first. The owner rejected that shape live at phase 1. The approved shape is `eventDaySeries()` (one row per event day) plus `hourSeries(dateIso)` (that single day's full 24 hours), switched by a `?day=` query param on one shared dropdown — never both charts on screen at once. `src/services/event-stats.js` owns both; `src/routes/admin/stats.js` picks which one to render and never computes a bucket itself.

**Why the day series is a union (configured range ∪ activity), not the configured range alone.** A guest can join, or a memory can post, on a day outside `getEventConfig()`'s `startDate`/`endDate` — an early RSVP, a destination-event guest arriving ahead of the configured window, or a post-wedding stray upload. If `eventDaySeries()` only walked the configured range, that activity would vanish from the chart while still counting in the stat-grid totals, so the day columns would silently stop summing to the numbers at the top of the page — the exact kind of "the chart doesn't add up" defect a host would notice and not be able to explain. `eventDaySeries()` instead unions `eventDays(startDate, endDate)`'s rows with every calendar day that carries at least one guest join or visible photo (issue #1022 AC3), so the sum invariant holds unconditionally, at the cost of an occasional extra column for an edge case that legitimately happened.

**Why the day-chart label falls back to "Aug 7" all-or-nothing, never a mixed axis.** The default 3-day weekend labels each column with its weekday abbreviation (`Fri`/`Sat`/`Sun`) — friendlier than a bare date for the common case. That breaks the moment two rows land on the same weekday, which the union rule above makes reachable (a 4-row series with an out-of-range Friday beside the configured one) and which any series past 7 rows guarantees (a 14-day destination event). The fallback is a single boolean computed once over the whole series — `hasDuplicateWeekday`, `src/services/event-stats.js`'s `eventDaySeries()` — not a per-row decision: either every row reads its short weekday, or every row reads the unambiguous `singleDayLabel()` form. A per-row fallback (only the colliding pair reads the long form) would produce an axis where two labels are drawn by two different rules with no visual cue why, which is worse than the collision it fixes.
...
## Ceremony photo notice: a broadcast independent of any task row, gated by date not by a live switch (#1042)

**Date:** 2026-08-02. **Status:** shipped.

**A fourth announcement source that names no task.** `src/services/notifications.js`'s own header comment stakes out the announcements source's boundary: "every fact an announcement asserts is already sitting on the task row itself" — true of the three derivations issue #778 shipped (a task going live, a one-day-only challenge unsealing, a flash window opening), each read off a candidate task row inside `qualifyingAnnounceFacts`'s per-task loop. The ceremony notice breaks that boundary on purpose: it names no task at all, reads two fields off `getEventConfig()` instead, and is derived once outside the loop rather than once per candidate task. `qualifyingAnnounceFacts` stays the one place this whole source is assembled — `announceRows` (fetch) and `announceCount` (count) still cannot disagree about which announcements exist, the same structural guarantee #778 built — but the header comment's boundary claim is now one derivation narrower than it reads; a future reader diffing that comment against the code should read this note, not conclude the comment is stale.

Two mechanical consequences of not being task-derived: (1) the row needed its own `kind` (`ceremony`) and `href` (`/no-photos`) instead of every announce row's shared `announce`/`/tasks` pair, which is why `qualifyingAnnounceFacts`' closing `facts.map(...)` was widened to read `f.kind`/`f.href` with a fallback to the old literals rather than hard-coding them — the three existing derivations push neither field, so they fall through byte-identical (regression-guarded in `tests/ceremony-notice.test.js`). (2) `getEventConfig()` is now read a second time per `qualifyingAnnounceFacts` call (five settings-table SELECTs instead of three, and this function runs twice per guest-page render — once via `announceCount`, once via `announceRows`) — accepted rather than threading the config through `buildRecapClock`, which is deliberately a pure function of one `timezone` argument with four call sites already depending on that shape.

**Date-driven, not a live switch, and why the explainer never 404s.** The couple cannot flip a toggle at the altar, so the host's entire involvement is picking one calendar day on `/admin/config` ahead of time — day granularity, no time-of-day window, the band stands all day on the configured date. This is a design choice, not a technical shortcut: a "ceremony is happening now" live switch would need someone backstage watching a clock during their own wedding. The corollary is that `/no-photos` (`src/routes/guest/pages.js`) renders its three copy paragraphs unconditionally, never checking the toggle at all (AC6) — a bookmarked link, or a stale recap row from a day the notice later got turned off, must never 404. The rule warns; it never blocks — uploads stay open on the ceremony day and nobody is locked out of the game mid-wedding.

**Review-round fix: one shared camera-slash glyph.** The camera-with-a-slash outline glyph shipped as four hand-copied inline SVGs (the day-of band, the how-to-play onboarding row, the no-photos hero, and the recap row's own `KIND_GLYPH.ceremony` entry) that had already diverged in stroke width and size. All four now render `src/views/partials/camera-slash.ejs`, the single owner of the drawing: the three view call sites via `<%- include('camera-slash') %>`, and `src/services/notifications.js`'s recap-row glyph via `ejs.compile`, the same pattern that module already uses for the badge-art partial. This also corrected a latent bug: the hand-written `KIND_GLYPH.ceremony` string carried no `fill="none"` on its root, so the recap icon rendered black-filled instead of the clean white outline every other call site showed.

## Point-system rebalance (2026-08-04)

**Date:** 2026-08-04. **Status:** in progress: #1103/#1104/#1105/#1106/#1107 ship the rows below one issue at a time, strictly sequentially, sharing several files (views, scoring modules, tests, docs) and each editing only its own rows/sections. This section is the one shared ADR home for the whole rebalance; each sibling issue records its own decisions in its own dated sub-heading below rather than opening a new top-level section.

**Why:** the owner's read of the first wave's play data was that a task photo, the hardest, most deliberate action in the game, paid the same or less than a passive like or an idle memory share, so the leaderboard rewarded volume over effort. The rebalance raises the floor under the highest-effort action (a completed task) so it reads, unambiguously, as the biggest single reward available, and adjusts several smaller sources around it so the whole point economy still adds up to a coherent story rather than a pile of independently-tuned numbers.

### Task worth widened to 3-5, default 3 (#1103)

**Decision:** `tasks.worth`'s range moves from `[1, 3]` (default 1) to `[3, 5]` (default 3): a straight `+2` shift, not a re-derivation, so a task set to the old minimum (1) lands on the new floor (3) and one set to the old maximum (3) lands on the new ceiling (5), and every relative ordering among existing tasks (which task pays more than which other) is preserved exactly, just rebased upward. `src/db/migrations-tasks.js`'s `ensureTaskWorthRange` is the guarded, idempotent rebuild that performs this remap on an existing database at boot; `src/db/schema.js`'s CHECK is the fresh-database shape. See that migration's own doc comment for why the copy must use an explicit column list rather than `SELECT *`: a migrated database's flash/lucky/live_since columns sit in different physical positions than a fresh one's.
...
**One choke point, not a route list.** `guests.blocked` (`src/db/migrations-guests.js`'s `ensureBlockedColumn`, the same guarded-column shape as `ensureIsCoupleColumn`/`ensurePinnedColumn`) is enforced in exactly two places: `attachGuest` (`src/middleware/session.js`), which every guest request passes through ahead of any route (`src/app.js` mounts it once), and `POST /login` (`src/routes/auth.js`), which needs its own check because sign-in mints a fresh cookie before `attachGuest` ever gets a chance to refuse it. Enforcing per-route instead, with a list of "guarded" routes to check `blocked` on, would need to be kept in sync by hand as routes are added, and a route added later without remembering the list would silently let a blocked guest act. One middleware plus one sign-in check covers upload, like, comment, and every future guest route with no new call site required.

**A refusal, not a takedown.** Blocking flips one column; it deletes nothing and hides nothing already contributed. `attachGuest` treats a blocked row as no session (`req.guest = null`, the `gsid` cookie cleared with the same `{ path: '/' }` shape `POST /logout` uses) but the row itself, and every submission/like/badge it owns, is untouched. The gallery and leaderboard render a blocked guest's past contributions exactly as before (AC6). This is the whole point of a checkbox instead of Delete: a host who blocked the wrong person can undo it, and nothing the guest already gave to the shared record was ever at risk.

**Sign-in fails closed, credential-check first, throttle-clear second.** `POST /login`'s blocked check sits after `pinOk` succeeds (so a blocked guest's _wrong_ PIN attempt looks like an ordinary wrong-PIN attempt to anyone timing responses, so no new existence-oracle) but before `guestLockoutState.delete(key)`, the line that clears a contact's per-attempt throttle on a correct login. Landing after that line would let a blocked guest keep clearing their own throttle by retrying with the correct PIN forever; landing before it is what the acceptance criterion calls out by name. A correct-credential attempt from a blocked guest is also not counted as a login failure (no `recordGuestLoginFailure` call): the credential was right, only the account is shut, and stacking a throttle penalty on top of a moderation decision would just make the eventual unblock-and-retry slower for no security benefit.

**Stated limits, not solved here.** Blocking is per `guests` row, not per person: a blocked guest can sign up again at `/join` under a different contact and get a fresh, unblocked row. Closing that gap means blocking a contact or a device, a different feature with its own design questions (what should the shared poster QR do for a blocked contact?), filed separately rather than folded in here. A guest blocked mid-session is also dropped silently rather than shown an explanation on the spot; the explanation lives at their next login attempt (the one guest-visible string this issue ships: "This account has been blocked. Please find one of the hosts."), because a mid-session explanation would need a new guest-facing screen state, which sits outside this issue's `src/views/**`-free `Touches`.

## `bug_reports.guest_id` dropped NOT NULL in place, not rebuilt (#1102)

**Date:** 2026-08-04. **Status:** shipped.

**Spawned by #1020's phase-1 loop.** #1020's approved guest-facing error page files a bug report on one tap from a guest who may not be signed in. `bug_reports.guest_id` was `NOT NULL`, so that insert could not happen at all, and `GET /admin/bugs` inner-joined `guests`, so even a stored null-guest row would have been invisible to the host. This issue is the storage shape and the host's Bugs queue only; the caller that files a signed-out report is #1020.

**One `ALTER TABLE ... ALTER COLUMN ... DROP NOT NULL`, not the 12-step rebuild.** `ensureTaskIdNullable` (`src/db/migrations-submissions.js`) predates this option and rebuilds `submissions` wholesale: `foreign_keys = OFF`, a transaction, a `CREATE TABLE ... _new`, an explicit column-copy `INSERT ... SELECT`, `DROP TABLE`, `RENAME`, and recreated indexes, because SQLite historically had no direct way to widen a NOT NULL column. This tree's `better-sqlite3@12.11.1` ships SQLite 3.53.2, which supports the direct `ALTER COLUMN ... DROP NOT NULL` statement. Executed against a replica of the real `bug_reports` shape, that single statement preserves the `guests(id) ON DELETE CASCADE` foreign key, the `status` CHECK, `idx_bug_reports_status`, and every existing row, and is idempotent. `ensureBugReportGuestIdNullable` (`src/db/migrations-ops.js`) therefore does none of the rebuild machinery: no `foreign_keys` pragma, no transaction, no column-copy list. An explicit copy list is the one thing that can permanently diverge a migrated database from a fresh one; this ALTER cannot produce that failure at all. The function is a thin PRAGMA-guarded wrapper around it, for an early exit on an already-migrated database, not because the statement is unsafe to repeat.

**No sentinel "anonymous" guest row.** The alternative to a nullable column was inserting one reserved `guests` row (id, say, 0) that every signed-out report points at, keeping `guest_id NOT NULL` and the `JOIN` intact. Rejected: it manufactures a guest that shows up in guest counts, guest exports, and any future guest-scoped query unless every one of those is separately taught to exclude it, a second fact to keep in sync forever, versus one column that is allowed to be empty. The nullable column pushes the "no guest" case to exactly the two places that already need to know about it: the query (`LEFT JOIN` instead of `JOIN`) and the view's `hasGuestId` predicate, the single owner both `reporterName` and the open row's link check call through.

**The query's own JOIN and projection both had to move.** `GET /admin/bugs` (`src/routes/admin/bugs-export.js`) changed `JOIN guests g` to `LEFT JOIN guests g`, and separately moved the `guest_id` projection from `g.id AS guest_id` to `r.guest_id AS guest_id`. The second change is not cosmetic: under a `LEFT JOIN`, `g.id` is NULL both for a genuinely guestless report and for a report whose `guest_id` points at a guest row that no longer exists, two different facts a projection from the guests side cannot tell apart. Projecting from the `bug_reports` side keeps `guest_id` truthful regardless of whether the join matched, which is what lets the view's `reporterName` helper (`src/views/admin-bugs.ejs`) render three distinct states: the guest's name, `Guest #<id>` for an unresolvable id, `Not signed in` for a genuinely null one, instead of collapsing the last two into the same bug the old `r.guest_name || ('Guest #' + r.guest_id)` expression had (a literal `Guest #null` string). That expression arrived with #686 and was hand-repeated at all four render sites; `reporterName` is this issue's, and is now their single owner.

## Guests screen rebuilt onto the Tasks card/dialog shape, one popup is the whole truth (#1093)

**Date:** 2026-08-04. **Status:** shipped.

**Why the Tasks shape, not a new one.** The old Guests screen rendered every control inline: a name form, an identity form (contact + PIN), a bonus-points form, a badge-award form and a Delete, all always open, per guest. At the seeded event's 40 guests that is roughly 200 controls and 40 red Delete buttons standing in the scroll path, and nothing about it could be scanned. The admin Tasks screen (`src/views/admin-tasks.ejs`) had already solved the same shape problem: a compact card per row opening one shared `<dialog>` holding every control, Delete included. Rather than grow a second dialog system with its own CSS and its own backdrop-dismiss wiring, Guests adopts Tasks's parts directly: `.task-dialog`, `.field`, `.admin-chip`, `dialog-dismiss.js`, and the `data-confirm` delete-proxy convention (the visible Delete button is a proxy that fires the row's own hidden real delete form via `requestSubmit()`, so `admin.js`'s delegated `data-confirm` listener still gates it). One dialog shape, one set of conventions, two consumers.

**Scope was cut live by the owner, not by this issue's own judgment.** During the phase-1 visual-approval loop the owner removed bonus points, gallery pinning, and badge awarding from this screen outright ("we have a system for badges"), along with the Dashboard back-link and Poster button (both already live in the top nav). What the popup keeps is exactly what a host does to a guest at the party: fix a name or contact, reset the code they log back in with, block them, remove them.

**The popup is the whole truth: a blank box is a stored value, never a "leave this alone" signal.** The screen used to split editing across two endpoints: `POST /guests/:id/edit` (name/pin/is_couple) and `POST /guests/:id/identity` (contact/PIN), and the identity route treated a blank contact or PIN as "skip this field", bailing out entirely with `Nothing to update.` if both were blank. That rule could not survive a single popup that pre-fills every box from the guest's stored values: the seeded event's own guests carry no contact and no PIN (`tests/helpers/event-fixture.js`'s `insertGuest` never sets either), so on a 40-guest screen almost every Save would post `contact=''` and `pin=''`, the _common_ case, not an edge one. Carrying the old skip-blank rule forward would make a rename-only or block-only save write nothing and answer `Nothing to update.`; writing all five fields unconditionally without the rule would silently wipe a real guest's contact and PIN the moment a host cleared a box. The rule this issue picked instead: the popup's five fields are the whole truth on every Save. A non-empty PIN/contact is validated (4 digits; normalizes and does not collide with another guest); a blank one writes `NULL` unconditionally and never blocks the other four fields. Validation runs before any write, so an invalid submission (bad PIN, colliding contact) leaves all five stored values untouched rather than half-applying the rest, the same all-or-nothing contract `/identity` already had, just widened to five fields instead of two. The shared half of that contract now has ONE owner rather than a copy per route: `resolveGuestIdentity` (module-level in `src/routes/admin/guests.js`, beside its only two consumers) answers "is this submitted contact/PIN pair acceptable for guest :id, and what should the host be told if not", and both `/edit` and `/identity` call it. It owns the validate-PIN-first ordering, the collision rule, and the exact failure wording, including the shared `CONTACT_TAKEN_MESSAGE` each route also uses for the UNIQUE-index catch around its own UPDATE (the pre-check is a check, not a lock, so the catch stays at each call site). The first draft of this change copied all of that into `/edit` and left `/identity` alone; a review caught that the collision rule and its wording then had no owner and were free to drift, which matters more than usual because both routes write the credentials `POST /login` authenticates against. What the two routes genuinely disagree about stays at the call site: what an empty field means.

**Three field states, not two.** A form field arrives as one of three things, and collapsing any two of them destroys data. `undefined` (the key was never submitted) means "leave this alone" on both routes, for the TEXT fields. Checkboxes take the opposite rule and have to: an unchecked box sends nothing at all, so absence is its only "off" signal, and `is_couple`/`blocked` therefore write 0 when the key is missing. The consequence is worth stating because it is the same shape as the `pinned` trap one paragraph up: a non-popup caller that omits those keys turns both flags off, so a request meaning only "rename this guest" also unblocks them. It is accepted here and not accepted for `pinned` for one reason: the approved popup renders a Blocked control and a couple control, so the popup, which is the only caller in the app, always states both; it renders no pinned control at all, so nothing would ever have stated that one. `''` (submitted empty) is a real value on `/edit` and writes `NULL`, but means "leave this alone" on `/identity`. A non-string value (Express hands back an ARRAY for a repeated key such as `contact=a&contact=b`) is a malformed submission and is refused outright. The absent case is not academic: the popup always sends every key, so only a non-popup caller omits one, and an in-repo caller does exactly that (`tests/couple-heart.test.js` posts name and is_couple only). Treating its absent keys as blank would have nulled that guest's contact and PIN and answered `Guest updated.`, which is how the first draft behaved until an architecture review probed it.

**The cost of that rule is accepted, not hidden, and no warning is added.** Contact and PIN together are a guest's only way back in on a new phone, so clearing the Contact box and saving takes that away as silently as clearing the Name box does. No hint is added under Contact for two reasons: the host who cleared it can retype it and re-save in the same box, so nothing is destroyed that the same hand can't undo; and the popup's pixels are owner-approved (phase-1 loop), so adding guest-facing copy to an approved screen as a side effect of this logic issue would be exactly the drift the phase-1 loop exists to prevent. Recorded here rather than silently accepted, and it is one line to add later if a host actually trips on it.

**`pinned` came out of the UPDATE, not just off the screen.** The old `POST /guests/:id/edit` wrote `pinned` from the same absent-key-is-false rule as `is_couple`: an unchecked box (or, before this issue, a box that simply doesn't exist on the form) unpins the guest. The approved popup has no pinned control at all, so carrying that rule forward would have silently unpinned every guest on every Save, the first time any host touched this screen after it shipped. `pinned` is dropped from the statement entirely rather than defaulted to the stored value in code, so the column is now unset by any endpoint (`src/services/feed.js` still reads it to hoist a guest's gallery section). A production check run on 2026-08-04 found no live guest carrying `pinned = 1` (recorded on the issue), and it is re-run immediately before merge rather than trusted from that reading, since the event is live; a guest pinned after this point can never be unpinned again without a raw DB write, a known and accepted limit rather than a bug. No issue tracks a replacement control: the owner cut gallery pinning from this screen by name and did not ask for it elsewhere, so this is a deliberate removal rather than a gap awaiting a fix. If it turns out to be missed, that is a new issue, not a forgotten one.
