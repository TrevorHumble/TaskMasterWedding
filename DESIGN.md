...
### Fable full self-certification exception (#203)

**Decision (2026-07-04):** by explicit owner direction, Fable-authored issues and PRs may be committed on Fable's own recorded PASS, at every risk tier — routine, system-level, security-flagged/high-stakes, and self-modification — with no independent reviewer required, permanently (no expiry, no time-boxed trial). This amends, for Fable only, the "Independence" and "Self-modification bar" sections of `standards/adversarial-review-protocol.md`: those sections continue to bind every other implementer unchanged. CI (`npm run lint`, `npm run format:check`, `npm run test:coverage`) is unaffected and remains the only automated check on Fable-authored work.

**Rationale:** the owner directed this exception directly, the same way #80 reversed the fixed-badge-catalog constraint by owner direction (see "Badge thresholds are config" above) and the 2026-07-02 Merge-policy decision retired the owner-merge boundary by owner direction. No architectural justification is offered beyond that direction — this is a deliberate, recorded loosening of the independence guarantee the rest of this document treats as load-bearing, not a discovery that independence was unnecessary.

**Mechanism:** `tools/persist-self-certification.ps1` writes `k` distinctly-tagged self-certification records (`reviewer_id` values `fable-self-1` … `fable-self-k`, `role: 'self-cert'`, `model` locked to `fable`) rather than exploiting the pre-existing free-text-`reviewer_id` gap silently. `Reduce-Verdicts` (`tools/verdict-core.ps1`) already counts distinct `reviewer_id`s with a PASS and does not read `role`, so this satisfies `Required` at any tier — including the system-level `Required = 2` bar — without any change to the shared counting kernel. `tools/persist-bias-gate.ps1`'s `-SelfCertify` switch writes a passing `bg1` artifact attributed to `fable-self` so a Fable-certified system-level tree also clears the bias-gate check in `tools/validate-verdict.ps1` without a separate independent bias-gate agent run.

**Honest bar:** this is not a new tamper-evident residual — it is the existing one (documented under "Commit gate" and "Issue-review gate" above: the orchestrator can already write free-text-`reviewer_id` evidence by hand) made explicit, labeled, and policy-sanctioned for Fable specifically, instead of left as an unlabeled gap any implementer could quietly exploit. Every other agent role remains bound by the unmodified Independence and Self-modification-bar rules.
