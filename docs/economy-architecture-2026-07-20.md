...
**Rule:** point terms are defined once, in one registry in `src/services/scoring.js`
(the same shape `src/services/badges.js` uses for badge registries), and BOTH read paths
compose the same term list. A new rule (worth, banked bonus, memory day, auto-badge +1,
crowd favorite, ranked awards) is one registry entry, never a second hand-written copy.
If the per-guest and all-guests shapes genuinely need different SQL, both shapes live
side by side in the term's single entry — never in two far-apart functions.
...
(#489/#490). A surface hand-composing badge art outside the partial is a defect. Gold has
two rank _sources_ (award rows for task badges, the derived crowd-favorite set) but one
_renderer_.

## The standing kills (context, owned elsewhere)

One badge substrate (`badges` + `guest_badges`; the placeholder five and their parallel
catalog die in #661's rewrite) · no freeform points (#683/#684) · no badges on people
(#683) · MOSTPHOTOS/MOSTLIKED removed (replaced by derived crowd favorite).
