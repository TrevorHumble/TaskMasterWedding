...
How to take Wedding Master from a bare Linux host to serving guests over HTTPS, with data that survives a restart. See [`DESIGN.md`](../DESIGN.md) § Hosted deployment for why this shape was chosen; this file is the how-to.
...
| `BASE_URL`                            | Yes                  | The public `https://` domain. The shared entry poster's QR code (`/admin/poster`) bakes in this value — get it right before printing the poster.                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `ERROR_REPORT_RATE_MAX`               | No                   | Per-key (guest, or IP when signed out) cap on `POST /error-report` (issue #1020, the error page's "Send error report" button), its own budget: never `RATE_LIMIT_SOCIAL_MAX`'s, so a guest stuck looping on a broken page can never burn the budget the human `POST /bug-report` channel depends on. Default `100` per window.                                                                                                                                                                                                              |
...
- `/admin/tasks` — create the photo tasks guests will complete.
- `/admin/guests` — create and edit guests (no QR codes to generate — the one printed QR is the shared entry poster at `/admin/poster`).
