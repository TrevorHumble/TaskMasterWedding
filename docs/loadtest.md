...
Control run: `server5xx=0`, `p95=7744ms`, **FAIL**, matching the harness's own printed verdict above. No explanation for the control run's higher p95 than the public run's was captured during this run, so none is asserted here. **The cause was identified after this section was first written**, by the independent measurement in issue #1140: per-request render cost grew roughly 6x between the 2026-07-18 build (`6e8602f`) and current main (`c95b78c`), spread across every page rather than concentrated in one. That issue's own hosted run recorded `p95 7154ms` at the same 100-concurrency shape, which matches the `7744ms` here. So the gap against the 2026-07-18 provisional run (`p95=1240ms` on this same private-network path) is a real regression in render cost, not measurement noise, and the ~30% CPU headroom that run had is spent. Restoring the margin is #1140's job; issue #1146 was filed from this run asking the same question and is closed as a duplicate of it.

Two caveats keep this from being a clean capacity verdict either way. The droplet now runs three app containers on 2 shared vCPUs and the driver ran in a fourth, neither measured against the result. And, per the subsection below, neither run completed an upload, so the mix is read paths plus cheap rejections with none of the multer and sharp cost, which makes both numbers here optimistic rather than pessimistic.
