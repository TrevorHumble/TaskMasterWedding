# Resume state — autonomous backlog run (historical)

**Historical snapshot.** This file records the state of the backlog-clearing run as of 2026-06-28 and is no longer updated. For live state, read the issue board (`gh issue list`), the ledger-branch BUILDLOG (`BUILDLOG.md`), and the live per-increment ledger at [`docs/live-log.md`](live-log.md). This is operational state, not product documentation — the North Star is [`north-star.md`](north-star.md); the standards are in `standards/`.
...
- **Couple-name spelling (resolved 2026-07-03):** the design system's spelling, **Lilly & Axel**, is the standing spelling. (The live-surface spelling sweep across guest-facing pages is tracked separately in #354.)
...
Relocated to [`docs/live-log.md`](live-log.md) — the live per-increment ledger of autonomous runs.
