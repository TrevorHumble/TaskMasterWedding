...
- **Photo-serving guard (corrected 2026-07-24, #833; path updated 2026-08-01, #979; ORIGINAL_RE/THUMB_RE re-pointed to naming.js, #979 PR-review fix round #2):** taken-down access control lives in `src/services/photos/moderation.js` (`blockTakenDownOriginal` / `blockTakenDownThumb`), mounted before the `/uploads` and `/thumbs` static mounts in `app.js`. The Stage-1 allowlist regexes live in `src/services/photos/naming.js`: `ORIGINAL_RE = ^[0-9a-f]{16}-\d+\.(jpg|png|webp)$` and `THUMB_RE = ^[0-9a-f]{16}-\d+\.(jpg|png|webp)\.jpg$` — **no `.heic` variant**: issue #281 converts every HEIC upload to JPEG before it is written under its final name, so a `.heic` file never reaches `UPLOADS_DIR` under a name this pattern would need to match. Stage-2 is `COLLATE NOCASE … taken_down = 1`. Any test fetching an image must use a realistic stored filename — the old `seed()` `p.jpg`/`t.jpg` do NOT match the allowlist (they 404, correctly).
