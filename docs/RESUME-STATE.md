...
- **Community pages are guest-gated** (403 without a `gsid` cookie); `/admin/login` is the one ungated view. Sign a guest in with `request.agent(app).get('/j/seedtoken')` (do NOT follow the redirect).
