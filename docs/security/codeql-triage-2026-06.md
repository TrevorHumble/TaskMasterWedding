...
- `src/routes/guest.js` avatar handler — `fs.readFileSync(req.file.path)` and
  `fs.unlinkSync(req.file.path)` (lines 427–440). `req.file.path` is multer's
  `destination + filename`; the `filename` value was generated by the callback
  above, not taken from the client.
...
handler (`src/routes/auth.js`, lines 166–) implements an in-process lockout:
...
Handlers that CodeQL flags (guest home, gallery, task list, leaderboard) are
either behind `requireGuest` / `requireAdmin` (so an unauthenticated attacker
cannot reach them at all) or are cheap in-process SQLite reads with no side
effects. The upload and export endpoints are auth-gated and size-capped.
