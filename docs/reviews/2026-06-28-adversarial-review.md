...
> **Resolution status (as of #847, 2026-07-24):** the vast majority of the findings below were
...
> auth/session model, and more — see `BUILDLOG.md` for the individual PRs). The remaining two,
> **Blocker-1, "No CSRF protection on any state-changing POST despite cookie-based auth,"** and
> **Major-12, "No security response headers (no helmet: CSP, X-Content-Type-Options,
> X-Frame-Options, Referrer-Policy),"** were RESOLVED under issue #284 (PR #845, which absorbed
> #560 and #769): the app now ships a signed double-submit CSRF token
> (`src/middleware/csrf.js`) plus three baseline security response headers in `src/app.js` —
> `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, and
> `Referrer-Policy: strict-origin-when-cross-origin`. Major-12's fourth item, a
> Content-Security-Policy, was deliberately NOT added — see `DESIGN.md` §
> "CSRF tokens and security headers: implemented (#284)" for the shipped mechanism and the
> reasoning behind leaving CSP out. This file is kept as the historical record of the original
> as-built review, not a live defect list.
